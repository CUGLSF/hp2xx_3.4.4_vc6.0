...
line_PicBuf(DevPt * p0, DevPt * p1, PEN_W pensize, PEN_C pencolor,
			...
			** Bresenham algorithm.//使用Bresenham算法在图片缓冲区中画一条线
			**/
{
	PicBuf *pb = po->picbuf;
	DevPt *p_act;
	...
	int dx, dy;
	int linewidth = (int) ceil(pensize * po->HP_to_xdots / 0.025);/*ceil()返回大于或者等于指定表达式的最小整数*/	
...
**/
/*从临时文件中读取绘图命令，把HP_GL坐标转换为点坐标，并且绘制(光栅化)矢量*/
{
	HPGL_Pt pt1;
	/*typedef struct 
	{
	...
	static DevPt ref = { 0, 0 };//ref为静态局部变量，函数调用结束后不释放存储空间
	DevPt next;
	PlotCmd cmd;
	...
	while ((cmd = PlotCmd_from_tmpfile()) != CMD_EOF)
		...
		case PLOT_AT:
			HPGL_Pt_from_tmpfile(&pt1);
			HPcoord_to_dotcoord(&pt1, &ref, po);
			line_PicBuf(&ref, &ref, pt.width[pen_no],//利用Bresenham algorithm绘制直线
				pt.color[pen_no], consecutive, po);
			consecutive = 0;
			break;
