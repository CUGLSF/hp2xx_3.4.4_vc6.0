...
void autoset_outfile_name(const char *mode, const char *in_name,
			  char **outfile)
{
	int len, i;
	...
	if (isascii(**outfile) && strlen(*outfile) > 0) //检查参数c是否为ASCII码字符, 也就是判断c的范围是否在0到127之间.
	{	/* If this looks like an output file name:     */
...
void cleanup(GEN_PAR * pg, IN_PAR * pi, OUT_PAR * po)
{
	cleanup_g(pg);
	cleanup_i(pi);
	cleanup_o(po);
...
int HPGL_to_TMP(GEN_PAR * pg, IN_PAR * pi)
{
   ...
   **/
	...
	if (*pi->in_file == '-')//char *in_file,如果Input file name( "-" = stdin )
		pi->hd = stdin;
	else if (pi->hd == NULL) 
	{
   ...
   **	 tmpfile() does not seem to work! //tmpfile不能正常工作，什么情况？
...
#if defined(DOS) && defined (GNU)
	if ((pg->td = fopen("hp2xx.$$$", "w+b")) == NULL)//w+b以读写方式打开文件，文件不存在则创建该文件。
#elif defined(AMIGA)
	if ((pg->td = fopen("hp2xx.tmp", "w+b")) == NULL)
#else
	if ((pg->td = tmpfile()) == NULL)//tmpfile()创建临时文件，创建成功返回指向该该文件的stream pointer,并赋值给FILE *td;
#endif	/** !DOS && GNU	**/
	{
...
int TMP_to_VEC(const GEN_PAR * pg, const OUT_PAR * po)
{
	if (pg->td == NULL)
		return ERROR;
	rewind(pg->td);		/* pg->td指向临时文件tmpfile.Rewind temp file for re-reading      */
	if (n_commands < 0)
		return 0;
 ...
 ** BUF_to_RAS ():
 **
 ** Conversion of raster picture buffer into the final output format.//把光栅图片缓冲区内容转化为最终的输出格式
 **
 ** Returns:
 **	ERROR	if error
