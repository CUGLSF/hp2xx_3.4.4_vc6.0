...
void Eprintf(const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	vfprintf(stderr, fmt, ap);//格式化数据并把结果按照format格式输出到指定文件中去。
...
void action_oldstyle(GEN_PAR * pg, IN_PAR * pi, OUT_PAR * po)//主要的操作处理函数
{
	int err;
	char savedname[100];
    /*int counter=-1;*/
...
/*	po->outfile=realloc(po->outfile,2*sizeof(char));*/
		...
		if (strcmp(pg->mode, "pre"))
		{
			...
			{
				sprintf(thepage, "%d", po->pagecount);//sprintf()将数据格式化输出到指定的缓冲区中
				strcpy(po->outfile, savedname);
				if (strstr(po->outfile, pg->mode))//strstr(*str1,*str2)实现从字符串str1中查找是否有str2
					strcpy(strstr
					       (po->outfile, pg->mode),
					       thepage);
				else
...
int main(int argc, char *argv[])
{
	...
	preset_par(&Pg, &Pi, &Po);//初始化通用参数，输入参数和输出参数信息
	if (argc == 1) 
	{
		usage_msg(&Pg, &Pi, &Po);
		exit(ERROR);
 ...
 **/
	...
	if (optind == argc) //optind:处理完一个参数后，下一个待处理参数(并不是命令行选项`-m`)的索引index.
	{	/*The variable optind is the index of the next element to be processed in argv. 
		 *The system initializes this value to 1.*/
		/* No  filename: use stdin        */
		Pi.in_file = "-";//输入的文件名字,Input file name ("-" = stdin) 
		autoset_outfile_name(Pg.mode, Pi.in_file, &Po.outfile);//OUT_PAR:-f outfile ("-" = stdout) 
		...
		{	/* Multiple-input file handling: */
			...
			/* if output file name given on command line, use it for all files */
			if (strlen(outname) > 0) 
			{
