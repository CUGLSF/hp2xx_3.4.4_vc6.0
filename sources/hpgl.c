...
FILE *td;//typedef struct{......FILE *td;/*(internally needed)*/}GEN_PAR;
...
static void init_HPGL(GEN_PAR * pg, const IN_PAR * pi)//static函数仅在本文件中使用
{
/**
** Re-init. global var's  for multiple-file applications
	**/
	/*fprintf(stderr,"init_HPGL\n");*/
	td = pg->td; /*pg->td指向创建成功的临时文件tmpfile.此处FILE *td是一个全局变量*/
...
void PlotCmd_to_tmpfile(PlotCmd cmd)
{
	...
	if (fputc((int) cmd, td) == EOF)/*int fputc(int n,FILE *fp)*/
	{
...
**/
{
	...
	for (;;) {
		if (read_float(&p.x, hd)) {	/* No number found      */
			if (numcmds > 0)
				return;
			if (pen_down && mv_flag && pen > 0 && pt.width[pen] < 0.35) {	/*simulate dot created by 'real' pen on PD;PU; */
				/*but not on PDPA */
				p.x = p_last.x + 0.01;
				p.y = p_last.y + 0.01;
				outside = 0;
...
static void tarcs(int relative, FILE * hd)
{
	...
	*/
	center.x =
		(2. * p2.y * (p3.x * p3.x + p3.y * p3.y) -
		2. * p3.y * p2.x * p2.x -
		2. * p3.y * p2.y * p2.y) / (2. * p3.x - 4. * p2.x * p3.y);
	center.y =
		(p2.x * p2.x + p2.y * p2.y -
		2. * p2.x * center.x) / (2. * p2.y);
...
**/
{
	...
	if ((c = (getc(pi->hd)) == EOF))//"=="优先级高于"="
		return;
	else
		ungetc(c, pi->hd);//ungetc()将读出的数据再次放回到缓冲区去，好像是撤销了前边的getc()操作
...
END:
	if (!pg->quiet && n_commands > 0) {
		...
		Eprintf("\nHPGL commands read: %d\n", n_commands);//变量n_commands计数读取的命令行数
		Eprintf("HPGL command(s) ignored: %d\n", n_unknown);//变量n_unknown计数未被成功解析的命令行数
		Eprintf("Unexpected event(s):  %d\n", n_unexpected);//变量n_unexpected对一些意外事件进行计数
		Eprintf("Internal command(s):  %ld\n", vec_cntr_w);//变量vec_cntr_w计数内部命令数目
		if ((pi->first_page > page_number - 1)//&&优先级高于||
